...
- **Storage.** SpecterX-protected data can be backed by Amazon S3, SharePoint, or Google Cloud Storage, in which case files live in the organization's own storage under SpecterX encryption. The choice is tenant-wide and applies to every kind of share — files uploaded from the web, attachments protected through Outlook or Gmail, Workspace contents, and exports from connected apps. SpecterX provides its own managed storage as the default.
