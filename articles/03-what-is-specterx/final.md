...
SpecterX is a data governance and secure collaboration platform. Your organization uses it to share files, send email, and collaborate with people outside the company while keeping control of what happens next: who opens the content, on what terms, for how long, and whether to revoke access at any time. The protection is built into each file and travels with it across applications, devices, and storage systems, so the rules you set still apply long after the file leaves your inbox, your drive, or your repository.
...
SpecterX sits between an organization's identity, storage, email, and security systems and the people it shares files with.

- **Identity.** SpecterX connects to the organization's identity provider, such as Microsoft Entra ID, Google Workspace, or Okta, so senders sign in with credentials they already have. SAML 2.0, OAuth, and LDAP are all supported. Recipients don't need any of this; they verify per share.
- **Storage.** SpecterX-protected data lives in the organization's own storage: Amazon S3, SharePoint, or Google Cloud Storage. An administrator picks the backend tenant-wide, and the choice applies to every kind of share: files uploaded from the web, attachments protected through Outlook or Gmail, Workspace contents, and exports from connected apps.
...
- **Data classification and DLP.** SpecterX plugs into the security stack the organization already runs. Policies can trigger a DLP or threat-detection scan when a recipient accesses a file. Policy assignment can also be driven by classification signals such as Microsoft Purview sensitivity labels, custom DLP headers from Exchange Online, or third-party classifiers, so the right protection is applied automatically as files move.
- **Deployment.** SpecterX runs in SpecterX's cloud by default, with an On-Prem Gateway option for organizations that need to keep the deployment in their own environment.

## What SpecterX is NOT

- **Not a storage product.** SpecterX is the protection and access-control layer for files. The files themselves live in the organization's own Amazon S3, SharePoint, or Google Cloud Storage, configured tenant-wide by an administrator. SpecterX adds governance to the storage the organization already runs.
- **Not a defense against a determined insider.** SpecterX prevents accidental forwarding, unauthorized access, and uncontrolled distribution from the SpecterX surface. It doesn't stop someone who can already see a file from photographing the screen or transcribing it. Watermarks deter that; they don't block it.
